...
const page = () => {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "graduation_year" && value.length > 4) {
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      setFieldErrors({
        graduation_year: ["Please enter a valid graduation year (1950-2030)"],
      });
    ...
    const contactNumber = formData.contactNumber.toString();
    if (contactNumber.length < 7 || contactNumber.length > 15) {
      setFieldErrors({
        contactNumber: ["Contact number must be between 7-15 digits"],
      });
      setError("Please correct the errors below.");
      setLoading(false);
      return;
    ...
    let noticePeriodBuyoutValue;
    const buyoutInput = formData.noticePeriodBuyout.toLowerCase().trim();
    if (buyoutInput === "yes" || buyoutInput === "true") {
      noticePeriodBuyoutValue = true;
    } else if (buyoutInput === "no" || buyoutInput === "false") {
      noticePeriodBuyoutValue = false;
    } else {
      setFieldErrors({ noticePeriodBuyout: ["Please enter 'Yes' or 'No'"] });
      setError("Please correct the errors below.");
      setLoading(false);
      return;
    }
    ...
    try {
      const payload = {
        ...
        noticePeriodBuyout: noticePeriodBuyoutValue,
        reasonForLeaving: formData.reasonForLeaving,
        school: formData.school,
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <input
                        ...
                        value={formData.contactNumber}
                        onChange={handleInputChange}
                        className={`w-full px-4 py-[15px] border-2 font-normal placeholder:text-[#63666A] ${
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      {/* <div>
                        <input
                          type="text"
                          name="noticePeriodBuyout"
                          placeholder="Notice Period Buyout Available* (Yes/No)"
                          value={formData.noticePeriodBuyout}
                          onChange={handleInputChange}
                          className={`w-full px-4 py-3 border-2 font-normal placeholder:text-[#63666A] ${
